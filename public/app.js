...
const state={project:{name:"",client:"",location:"",date:"",intro:"",cover:""},rooms:[{id:"r1",title:"SDB MASTER",subtitle:"Robinetterie & sanitaires",manual:[]}],selected:[],commercial:{clientDiscount:0,vatRate:20,shippingFee:0,supplierDiscounts:{"Amphora":50,"Catalano":64,"Coalbrook":50,"Zucchetti":55,"Lefroy Brooks":50,"Hotbath":50,"Recor":61.5385}}};
...
function loadState(){
  ...
  if(!state.commercial || typeof state.commercial!=="object"){
    state.commercial={clientDiscount:0,vatRate:20,shippingFee:0,supplierDiscounts:{}};
  }
...
function updateDependentFilters(resetCollection=false){
 const manufacturer=$("#manufacturerFilter").value;
 fillSelect("collectionFilter",valuesFor("collection",manufacturer),!resetCollection);
...
function projectFinancials(){
  ...
  const net=netBeforeShipping+shipping;

  const purchaseProducts=projectPurchaseCost();
  const purchase=purchaseProducts+shipping;
  ...
  return {
    ...
    productsNet,manualNet,shipping,net,
...
function renderMarginDashboard(){
  ...
  const cd=$("#clientDiscount"), vat=$("#quoteVatRate"), ship=$("#shippingFee");
...
function quoteRows(){
  ...
  for(const r of state.rooms){
    ...
    roomProducts.forEach(p=>{
      ...
      const freight=Math.max(0,Number(p.mandatoryFreight)||0);
      const key=[p.reference,p.finish,p.manufacturer,discount,freight].join("|");
      if(!grouped.has(key)) grouped.set(key,{...p,qty:0,_effectiveDiscount:discount,_freight:freight});
      grouped.get(key).qty++;
    });
    ...
    const mg=new Map();
    (r.manual||[]).forEach(m=>{
      const key=[m.label,Number(m.price||0)].join("|");
    ...
    for(const m of mg.values()){
      ...
      rows.push({
        ...
        freight:0,manual:true,discount:clientDiscountRate()
      });
    }
  }
  return rows;
  ...
  for(let pg=0;pg<pages;pg++){
    ...
    html+=`<section class="page quote-page">
      ...
      <table class="quote-table">
        <thead><tr><th>Pièce</th>${state.showSupplierReferences!==false?`<th>Référence</th>`:""}<th>Désignation</th><th>Qté</th><th>PU HT</th><th>Remise</th><th>Total HT</th></tr></thead>
        <tbody>${chunk.map(row=>`
          <tr>
            <td>${row.room}</td>
          ...
          </tr>`).join("") || `<tr><td colspan="${state.showSupplierReferences!==false?7:6}">Aucun produit sélectionné.</td></tr>`}
        </tbody>
      </table>
...
function buildDocument(){
 ...
 state.rooms.forEach(r=>{
   ...
   products.filter(p=>p.includeInstallationGuide&&p.installationGuideUrl&&/\.pdf(?:\?|$)/i.test(p.installationGuideUrl)).forEach(p=>{
     html+=technicalDocumentPage(r,p,p.installationGuideUrl,"Notice d’installation",no++);
   });
...
function harmonizeSavedCatalogProducts(){
  ...
  for(const p of state.selected||[]){
    ...
    const runtime={
      ...
      drawingUrl:p.drawingUrl,drawingType:p.drawingType,drawingLabel:p.drawingLabel,
      ...
      customImage:p.customImage,clientDiscountOverride:p.clientDiscountOverride
    };
...
$("#manufacturerFilter").addEventListener("input",()=>{
