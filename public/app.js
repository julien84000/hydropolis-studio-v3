...
const state={project:{name:"",client:"",location:"",date:"",intro:"",cover:""},rooms:[{id:"r1",title:"SDB MASTER",subtitle:"Robinetterie & sanitaires",manual:[]}],selected:[],commercial:{clientDiscount:0,vatRate:20,shippingFee:0,supplierDiscounts:{"Amphora":50,"Catalano":64,"Coalbrook":50,"Zucchetti":55}}};
...
function loadState(){
  ...
  const defaultSupplierDiscounts={"Amphora":50,"Catalano":64,"Coalbrook":50,"Zucchetti":55};
...
function effectiveSaleValue(p){
  return Number(p?.totalPrice||0)*(1-effectiveDiscountRate(p)/100);
}
...
function purchaseCostFor(p){
  return Number(p.totalPrice||0)*(1-supplierDiscountRate(p.manufacturer)/100);
}
...
function commercialPriceHtml(value,p=null){
  ...
  if(!d) return `${euro(value)} HT`;
  const net=Number(value||0)*(1-d/100);
  ...
  return `<span class="price-list">${euro(value)} HT</span><span class="price-discount">${label}</span><strong>${euro(net)} HT</strong>`;
}
...
function renderMarginDashboard(){
  ...
  if(supplierHost){
    supplierHost.innerHTML=makers.length?makers.map(m=>`
      <label class="supplier-discount-row">
        <span><b>${m}</b><small>Remise achat fournisseur</small></span>
  ...
  if(detail){
    const rows=makers.map(m=>{
      ...
      return `<tr><td>${m}</td><td>${ps.length}</td><td>${euro(list)}</td><td>${supplierDiscountRate(m).toLocaleString("fr-FR",{maximumFractionDigits:1})}%</td><td>${euro(cost)}</td><td>${euro(sale)}</td><td class="${mg<0?"negative":""}">${euro(mg)} <small>${tm.toLocaleString("fr-FR",{maximumFractionDigits:1})}%</small></td></tr>`;
    }).join("");
...
function quoteRows(){
  ...
  for(const r of state.rooms){
    ...
    roomProducts.forEach(p=>{
      ...
      const key=[p.reference,p.finish,p.manufacturer,discount].join("|");
      if(!grouped.has(key)) grouped.set(key,{...p,qty:0,_effectiveDiscount:discount});
      grouped.get(key).qty++;
    });
    for(const p of grouped.values()){
      rows.push({
        room:r.title,reference:p.reference,designation:p.designation,finish:p.finish,
        manufacturer:p.manufacturer,qty:p.qty,unit:Number(p.totalPrice||0),
    ...
    for(const m of mg.values()){
      rows.push({
        ...
        manufacturer:"Élément libre",qty:m.qty,unit:Number(m.price||0),
      ...
      <table class="quote-table">
        ...
        <tbody>${chunk.map(row=>`
          <tr>
            ...
            <td><b>${row.designation}</b><small>${row.manufacturer}${row.finish?` · ${row.finish}`:""}</small></td>
            <td>${row.qty}</td>
            <td>${euro(row.unit)}</td>
            <td>${row.discount?`${row.discount.toLocaleString("fr-FR",{maximumFractionDigits:1})}%`:"—"}</td>
            <td>${euro(row.unit*row.qty*(1-row.discount/100))}</td>
   ...
   <div class="room-products">${ps.length?ps.map(p=>`
      <div class="room-product">
        ...
        <div class="article-discount-panel">
          ...
          <b>${euro(effectiveSaleValue(p))} HT net</b>
        </div>
