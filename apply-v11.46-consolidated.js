...
idx=idx.replace(/Hydropolis Studio V11\.45/g,"Hydropolis Studio V11.46").replace(/<em>V11\.45<\/em>/g,"<em>V11.46</em>");
...
if(!idx.includes("v1146.css"))idx=idx.replace(/<link rel="stylesheet" href="styles\.css(?:\?[^\"]*)?">/,m=>m+'\n<link rel="stylesheet" href="v1146.css?v=11.46">');
if(!idx.includes("v1146-pricing.js"))idx=idx.replace(/<script src="app\.js(?:\?[^\"]*)?"><\/script>/,'<script src="app.js?v=11.46"></script>\n<script src="v1146-pricing.js?v=11.46"></script>\n<script src="v1146.js?v=11.46"></script>');
else idx=idx.replace(/<script src="app\.js(?:\?[^\"]*)?"><\/script>/,'<script src="app.js?v=11.46"></script>');
write("public/index.html",idx);
let sw=read("public/sw.js").replace(/hydropolis-v11-45-shell/g,"hydropolis-v11-46-shell").replace(/hydropolis-v11-45-catalogs/g,"hydropolis-v11-46-catalogs");
...
let server=read("server.js").replace(/Hydropolis Studio V11\.45/g,"Hydropolis Studio V11.46").replace(/Hydropolis V11\.45/g,"Hydropolis V11.46");write("server.js",server);
let cm=JSON.parse(read("public/catalog_manifest.json"));cm.version="11.46.0";cm.engineVersion="11.46.0";write("public/catalog_manifest.json",JSON.stringify(cm,null,2)+"\n");
let mm=JSON.parse(read("public/manufacturers_manifest.json"));mm.version="11.46.0";for(const m of mm.manufacturers||[]){if(m.name==="Resigres"&&!m.capabilities.includes("automatic-pricing-2026"))m.capabilities.push("automatic-pricing-2026")}write("public/manufacturers_manifest.json",JSON.stringify(mm,null,2)+"\n");
let pkg=JSON.parse(read("package.json"));pkg.version="11.46.0";pkg.description="Hydropolis Studio V11.46 - tarifs Resigres 2026 intégrés et calcul automatique";pkg.scripts.start="node server.js";
...
console.log("Hydropolis Studio V11.46 consolidée : OK");
